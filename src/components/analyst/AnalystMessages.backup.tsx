...
const AnalystMessages: React.FC<AnalystMessagesProps> = ({ selectedConversationId }) => {
  ...
  const projectConversations = conversations.filter(conv => conv.opportunity_id !== null);
  const generalConversations = conversations.filter(conv => conv.opportunity_id === null);

  // Filter by search term
  const filteredProjectConversations = projectConversations.filter(conv => 
    conv.opportunity?.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
    conv.creator.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    conv.lastMessage?.content.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const filteredGeneralConversations = generalConversations.filter(conv => 
    conv.creator.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    conv.creator.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
    conv.lastMessage?.content.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const currentConversations = activeTab === 'projects' 
      ...
      <div className={`${selectedConversation ? 'hidden md:block' : 'block'} w-full md:w-1/3 border-r border-gray-200`}>
        <div className="p-4 border-b border-gray-200">
          ...
          <div className="flex mb-4 bg-gray-100 rounded-lg p-1">
            ...
            >
              Projetos ({projectConversations.length})
            </button>
            <button
            ...
            >
              Avulsas ({generalConversations.length})
            </button>
          </div>
          ...
          {currentConversations.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-gray-500">
              ...
              <p className="text-center px-4">
                {activeTab === 'projects' 
                  ? 'Nenhuma conversa de projeto encontrada'
                  : 'Nenhuma conversa avulsa encontrada'
                }
              </p>
              {activeTab === 'projects' && (
                <p className="text-xs text-center px-4 mt-2">
                  Conversas são criadas automaticamente quando creators são aprovados para projetos
                </p>
              )}
              {activeTab === 'general' && (
                <p className="text-xs text-center px-4 mt-2">
                  Inicie conversas com creators pela tela de "Criadores"
                </p>
              )}
            </div>
          ) : (
              ...
              >
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Project title for project conversations */}
                    {conversation.opportunity_id && conversation.opportunity && (
                      <p className="text-xs text-purple-600 font-medium truncate mb-1">
                        📁 {conversation.opportunity.title}
                      </p>
                    )}
                    
                    {/* General conversation indicator */}
